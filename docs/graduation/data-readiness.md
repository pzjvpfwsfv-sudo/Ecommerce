...
行为链路按 G2-A 可恢复回放、G2-B DataStream 质量入口、G2-C Iceberg 落湖和 G2-D 版本化指标 API 分阶段实施；截至 2026-09-19，四个阶段均已完成小规模真实正确性验收。G2-E 已于 2026-09-24 完成独立 Olist 订单域全量九文件验收。下一步进入 G3 前端与 G4 知识库；RAG 必须与最终指标口径同版本，不提前把待确认口径发布为可信知识，也不跨两个匿名数据集推断用户旅程。
...
G2-D 已将 Snapshot `881836466779140976` 发布为 `behavior-v1-s881836466779140976`。Trino 源总数、Doris `DAY/FULL` 总数和 API overview 均为 1,002，`clean=1,001`、`late=1`、不同 `event_id=1,002`；四张 Doris 指标表行数为 `2/2/1,780/1`，重算 SHA-256 均与发布元数据一致，六个 API 均返回 HTTP 200。`purchase_amount_proxy` 仅是购买事件价格字段的分析代理值，不具备数量、币种、折扣、退款、取消或支付状态语义。该 run 是最终 SQL/verifier 加固前的不可变历史证据，既有发布未刷新或覆盖；加固后的动态发布验收必须等待自然产生的新 Snapshot。完整证据与重跑边界见 [G2-D 行为指标与版本化 API 验收记录](behavior-metrics-api-runbook.md)。本次 1,002 条结果明确是非容量证据。最新一次 G2-D 阶段完整自动化回归为 `558` 项通过，耗时 `186.763` 秒。

## G2-E Olist 订单域验收

Olist 官方 Version 2 ZIP 为 44,717,580 字节，SHA-256 为 `967e41e04fc306fe604e2a693f488995a8b41e5047418f8a5c8e4abd6deca784`。九个 CSV 共 1,550,922 条逻辑记录、126,186,995 字节，规范化后仍为 1,550,922 条；bundle 身份为 `3e0119b83f4ae47a992a6b2dcf30a6ed57403ef798413209ed52d2d4e624c3c3`。每个实体都以独立有限 Flink SQL 作业写入 Iceberg，源/规范化/入湖行数、不同稳定行 ID、顺序摘要和正 Snapshot 全部一致。

九张主题表严格保持业务粒度：订单 99,441、商品明细 112,650、支付 103,886、评价源记录 99,224，五张维表也分别与预期和不同键数一致。18 项重复键、孤儿键、空关键 ID、混合 bundle、负金额和非法时间硬门禁全为 0；真实质量问题不隐藏，包含重复 `review_id=789`、缺失商品品类 610、缺失翻译 13、一单多评价 547、缺失可选时间 2,980、生命周期异常 1,382、支付与商品加运费不一致订单 576。

不可变发布 run 为 `orders-v1-b3e0119b83f4ae47a992a6b2dcf30a6ed57403ef798413209ed52d2d4e624c3c3`，订单窗口为 `2016-09-04` 至 `2018-10-17`。Doris 六类指标行数为 `660/660/3,031/316,153/660/1`，总计 321,165；候选文件摘要、Doris 回读摘要、Snapshot 元数据、DAY/MONTH/FULL 查询和八个 API 合同全部通过，最终证据状态为 `PASS`。最终全仓回归通过 658 项、耗时 237.357 秒，1 项因 Windows 主机不支持符号链接而按设计跳过；Compose 渲染和差异检查通过。金额仍无已核实币种、退款和会计语义，不能称为利润、净收入或审计 GMV。完整 Snapshot、SHA-256、资源峰值与安全重跑方式见 [G2-E Olist 订单域真实验收与运行手册](olist-order-domain-runbook.md)。
