...
后续接入按 G2-A 可恢复回放、G2-B DataStream 质量入口、G2-C Iceberg 落湖和 G2-D 版本化指标 API 分阶段实施；截至 2026-09-18，前三阶段已完成小规模真实正确性验收，下一步进入 G2-D。各业务分组的有效样本量、会话边界及留存分母规则须在相应指标上线前验收。随后接前端与知识库；RAG 必须与最终指标口径同版本，不提前把待确认口径发布为可信知识。
