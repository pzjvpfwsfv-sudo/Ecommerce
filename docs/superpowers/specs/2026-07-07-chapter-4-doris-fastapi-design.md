...
2026-07-14 的全链路复验进一步确认：脚本不能只保证“命令执行过”，还必须保证依赖按正确顺序进入业务可用状态。

- **先重建、后初始化**：无持久卷的 Doris 容器被 `--force-recreate` 后会丢失表结构，因此初始化必须位于完整栈启动之后。
- **静态地址与动态池隔离**：Doris FE/BE 的固定地址保留在低位地址段，普通服务只允许从 `172.21.80.128/25` 动态分配，避免并发启动时抢占固定地址。
- **分层判断就绪**：容器 Running、FE 端口可连接、BE Alive、Kafka topic 存在是四个不同条件，不能相互替代。
- **业务资源幂等创建**：`analytics.realtime_metrics` 与 `user_behavior_events` 都由脚本显式、可重复地创建，不能依赖容器旧状态或 Kafka 自动建主题。
- **以数据闭环验收**：最终标准仍是 Flink 持续运行、Doris 可查到 PV/UV、FastAPI 返回与 Doris 一致，而不是以 Compose 启动成功作为完成标志。

这些约束让第 4 章的启动脚本具备可重复执行能力，也为后续生产化演进提供了明确方向：持久化存储、健康检查、可观测性和更严格的服务编排。
