...
class Chapter6TrinoArtifactsTest(unittest.TestCase):
    ...
    def test_trino_catalog_points_to_minio_iceberg(self) -> None:
        ...
        self.assertIn("iceberg.catalog.type=hive_metastore", catalog_text)
        self.assertIn("hive.metastore.uri=thrift://hive-metastore:9083", catalog_text)
        self.assertIn("fs.native-s3.enabled=true", catalog_text)
        ...
        self.assertIn("s3.aws-access-key=${ENV:MINIO_ROOT_USER}", catalog_text)
        self.assertIn("s3.aws-secret-key=${ENV:MINIO_ROOT_PASSWORD}", catalog_text)
        self.assertNotIn("fs.s3.enabled=true", catalog_text)
        self.assertNotIn("iceberg.catalog.type=hadoop", catalog_text)
        self.assertNotIn("iceberg.hadoop.warehouse=s3a://warehouse/iceberg", catalog_text)
    ...
    def test_verification_script_checks_nonzero_rows(self) -> None:
        ...
        self.assertIn("hive-metastore", script_text)
        self.assertIn("nextUri", script_text)
        self.assertIn("Expected exactly 2 non-empty SQL statements", script_text)
        self.assertIn("Trino diagnostics:", script_text)
        self.assertIn("Wait-ForTrinoStatementReady", script_text)
        self.assertNotIn("Invalid value 'hadoop' for type CatalogType", script_text)
        self.assertNotIn("shared supported catalog", script_text)
    ...
    def test_docs_mention_chapter6_trino_validation(self) -> None:
        ...
        self.assertIn("HadoopCatalog", readme_text)
        self.assertIn("Hive Metastore", readme_text)
        self.assertIn("Flink 与 Trino 共用", readme_text)
        self.assertNotIn("真正读通前还需要演进", readme_text)
        ...
        self.assertIn("HadoopCatalog", jobs_text)
        self.assertIn("Flink 与 Trino 共用", jobs_text)
        self.assertNotIn("真正读通前还需要演进", jobs_text)
